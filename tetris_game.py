...
class Tetris(QMainWindow):
    def __init__(self):
        ...
        self.isPaused = False
        self.nextMove = None
        self.lastShape = Shape.shape_none
        ...
        if event.timerId() == self.timer.timerId():
            ...
            if self.nextMove:
                ...
                while BOARD_DATA.currentDirection != self.nextMove[0] and k < 4:
                    ...
                    k += 1
                k = 0
                while BOARD_DATA.current_X != self.nextMove[1] and k < 5:
                    if BOARD_DATA.current_X > self.nextMove[1]:
                        BOARD_DATA.moveLeft()
                    elif BOARD_DATA.current_X < self.nextMove[1]:
                        BOARD_DATA.moveRight()
                    k += 1
            # lines = BOARD_DATA.dropDown()
            lines = BOARD_DATA.moveDown()
            self.tboard.score += lines
            if self.lastShape != BOARD_DATA.current_shape:
                self.nextMove = None
                self.lastShape = BOARD_DATA.current_shape
            self.updateWindow()
        else:
            super(Tetris, self).timerEvent(event)

    def keyPressEvent(self, event):
        if not self.isStarted or BOARD_DATA.current_shape == Shape.shape_none:
...
class SidePanel(QFrame):
    ...
    def paintEvent(self, event):
        ...
        minX, maxX, minY, maxY = BOARD_DATA.next_shape.getBoundingOffsets(0)

        dy = 3 * self.gridSize
        dx = (self.width() - (maxX - minX) * self.gridSize) / 2

        val = BOARD_DATA.next_shape.shape
        for x, y in BOARD_DATA.next_shape.get_coords(0, 0, -minY):
...
class Board(QFrame):
    ...
    def paintEvent(self, event):
        ...
        for x, y in BOARD_DATA.getCurrentShapeCoord():
            val = BOARD_DATA.current_shape.shape
            drawSquare(painter, x * self.gridSize, y * self.gridSize, val, self.gridSize)
